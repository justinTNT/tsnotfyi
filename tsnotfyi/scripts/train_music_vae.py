...
def load_config(config_path: str) -> Dict:
    ...
    # Modern config stores a connection string only
    if 'connectionString' in pg_config and pg_config['connectionString']:
        url = urlparse(pg_config['connectionString'])
        if url.scheme not in ('postgresql', 'postgres'):
            raise ValueError(f"Unsupported connection string scheme: {url.scheme}")

        username = url.username or pg_config.get('user')
        password = url.password or pg_config.get('password')
        host = url.hostname or pg_config.get('host', 'localhost')
        port = url.port or pg_config.get('port', 5432)
        database = url.path.lstrip('/') if url.path and url.path != '/' else pg_config.get('database')

        if not database:
            raise ValueError('Database name missing from connection string/config')

        return {
            'host': host,
            'port': port,
            'database': database,
            'user': username or os.getenv('PGUSER') or 'postgres',
            'password': password or pg_config.get('password')
        }

    # Legacy explicit fields
    required_keys = ['host', 'port', 'database', 'user']
    missing = [key for key in required_keys if key not in pg_config]
    if missing:
        raise KeyError(f"Missing PostgreSQL config keys: {', '.join(missing)}")
