...
def save_embeddings_to_database(identifiers, embeddings, database_type='postgres', db_path=None, pg_config=None, model_version=None):
    ...
    # Prepare update data (live schema uses vae_latent_*)
    vae_columns = [f'vae_latent_{i}' for i in range(8)]
    ...
    if database_type == 'postgres':
        with conn.cursor() as cur:
            for i, identifier in enumerate(identifiers):
                ...
                metadata_clause = ', vae_model_version = %s, vae_computed_at = CURRENT_TIMESTAMP'
                query = f"UPDATE music_analysis SET {set_clause}{metadata_clause} WHERE identifier = %s"

                cur.execute(query, list(embedding) + [model_version or f"vae_{int(time.time())}", identifier])
        ...
        for i, identifier in enumerate(identifiers):
            ...
            metadata_clause = ', vae_model_version = ?, vae_computed_at = CURRENT_TIMESTAMP'
            query = f"UPDATE music_analysis SET {set_clause}{metadata_clause} WHERE identifier = ?"
...
def main():
    ...
    try:
        ...
        model_version = f"vae_{int(time.time())}"
        ...
        logger.info(f"✅ Embeddings tagged with model version {model_version}")
